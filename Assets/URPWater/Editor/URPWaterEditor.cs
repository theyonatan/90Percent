...
public class URPWaterEditor : ShaderGUI
{
    ...
    bool[] _WaveVisible = new bool[] { true, true, true, true }; 

    Gradient _RefractionGradient = new Gradient();
    List<GradientColorKey> _GradientColorKeys = new List<GradientColorKey>();

    
    public List<URPWaterWaveDefinition> _Waves = new List<URPWaterWaveDefinition>();

    URPWaterWaves _WaveAsset;
    ...
    void DrawFoam()
    {
        ...
        var displacementEnabled = _Target.IsKeywordEnabled("_DISPLACEMENTMODE_GERSTNER") || _Target.IsKeywordEnabled("_DISPLACEMENTMODE_GERSTNERPLUS");
        if (displacementEnabled && enableFoam.floatValue == 1)
        {

            _Editor.ShaderProperty(whiteCaps, "White Caps");
        }

        if (whiteCaps.floatValue == 1 && displacementEnabled && enableFoam.floatValue == 1)
        {
    ...
    void DrawScattering()
    {
        ...
        if (_Target.IsKeywordEnabled("_DISPLACEMENTMODE_GERSTNER") || _Target.IsKeywordEnabled("_DISPLACEMENTMODE_GERSTNERPLUS"))
        {
    ...
    void DrawDisplacement()
    {
        ...
        // Gerstner Wave
        if (displacementMode.floatValue != 0.0f)
        {
            ...
            EditorGUI.indentLevel += 2;
            switch (displacementMode.floatValue)
            {
                case 1:
                    DrawGersnerWave();
                    break;
    ...
    void DrawGerstnerPlus()
    {
        var clusterCount = FindProperty("_ClustersCount");
        var wavesCount = FindProperty("_WavesPerCluster");
        var amplitude = FindProperty("_WaveAmplitude");
        int count = (int)clusterCount.floatValue;

        EditorGUILayout.Space(4);
        DrawIntSlider(clusterCount, "Custers Count", 1, 4);
        DrawIntSlider(wavesCount, "Waves Per Cluster", 1, 4);
        _Editor.ShaderProperty(amplitude, "Amplitude");
        EditorGUILayout.Space(8);

        EditorGUI.indentLevel -= 3;

        var boxStyle = new GUIStyle(EditorStyles.helpBox);
        boxStyle.margin = new RectOffset(70, 0, 5, 5);
        boxStyle.padding = new RectOffset(16, 16, 8, 8);

        var spread = FindProperty("_WaveOffsets");

        for (int i = 1; i <= count; i++)
        {
            var wave = FindProperty("_WaveParams" + i);
            var index = i - 1;
            GUILayout.BeginVertical(boxStyle);
            _WaveVisible[index] = EditorGUILayout.Foldout(_WaveVisible[index], "Cluster " + i, _FoldoutStyle);

            if (_WaveVisible[index])
            {
                DrawWaveParameters(wave, spread, index); 
            }
    ...
    void DrawIntSlider(MaterialProperty prop, string newName, int min, int max)
    {
        var floatValue = prop.floatValue;
        var a = EditorGUILayout.IntSlider(newName, (int)floatValue, min, max);

        prop.floatValue = (float)a;
    }
    ...
    void Draw4FloatFromVector4(MaterialProperty prop, string newNameA, string newNameB, string newNameC, string newNameD )
    {
        var vectorValues = prop.vectorValue;

        var floatA = vectorValues.x;
        var floatB = vectorValues.y;
        var floatC = vectorValues.z;
        var floatD = vectorValues.w;

        var a = EditorGUILayout.FloatField(newNameA, floatA);
        var b = EditorGUILayout.FloatField(newNameB, floatB);
        var c = EditorGUILayout.FloatField(newNameC, floatC);
        var d = EditorGUILayout.FloatField(newNameD, floatD);

        prop.vectorValue = new Vector4(a, b, c, d);
    }


    void DrawWaveParameters(MaterialProperty waveProperties, MaterialProperty spread, int index)
    {
        var vectorValues = waveProperties.vectorValue;
        var spreadValue = spread.vectorValue[index];

        var length = vectorValues.x;
        var steepness = vectorValues.y;
        var speed = vectorValues.z;
        var direction = vectorValues.w;

        var x = EditorGUILayout.FloatField("Length", length);
        var y = EditorGUILayout.Slider("Steepness", steepness, 0.0f, 2.0f);
        var z = EditorGUILayout.FloatField("Speed", speed);
        var w = EditorGUILayout.Slider("Direction", direction, 0.0f, 1.0f);
        var o = EditorGUILayout.FloatField("Spread", spreadValue);

        waveProperties.vectorValue = new Vector4(x, y, z, w);

        var xyzw = spread.vectorValue;
        xyzw[index] = o;
        spread.vectorValue = xyzw;
    }
